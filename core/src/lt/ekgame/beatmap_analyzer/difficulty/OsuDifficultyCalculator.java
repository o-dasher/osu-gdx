...
public class OsuDifficultyCalculator implements DifficultyCalculator {
	...
	public OsuDifficulty calculate(Mods mods, Beatmap beatmap) {
		...
		Array<OsuObject> hitObjects = bm.getHitObjects();
		Array<DifficultyObject> objects = generateDifficultyObjects(hitObjects, bm.getCS(mods), timeRate);
		
		Array<Double> aimStrains = calculateStrains(objects, DIFF_AIM, timeRate);
		Array<Double> speedStrains = calculateStrains(objects, DIFF_SPEED, timeRate);
		
		Array<Double> aimStrainsOriginal = new Array<>(aimStrains);
		Array<Double> speedStrainsOriginal = new Array<>(speedStrains);
	...
	private Array<Double> calculateStrains(Array<DifficultyObject> objects, byte difficultyType, double timeRate) {
	...
	public double calculateDifficulty(Array<Double> strains) {
		strains.sort((a, b) -> (int) (Math.signum(b - a)));
